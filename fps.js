...
import { createErlingRig, animateErling, disposeErlingRig, addSchoolWallArt } from './fps-visuals.js?v=20260905-art1';
...
function makeTransparentErlingTexture(image){
  ...
  const nearWhite=i=>{const p=i*4,r=data[p],g=data[p+1],b=data[p+2];return r>215&&g>207&&b>185&&Math.max(r,g,b)-Math.min(r,g,b)<45;};
  const push=(x,y)=>{if(x<0||y<0||x>=w||y>=h)return;const i=y*w+x;if(seen[i]||!nearWhite(i))return;seen[i]=1;stack.push(i);};
  for(let x=0;x<w;x++){push(x,0);push(x,h-1);}for(let y=0;y<h;y++){push(0,y);push(w-1,y);}
  // The paper also shows through the enclosed spaces under his bent arms.
  push(Math.floor(w*.30),Math.floor(h*.49));push(Math.floor(w*.68),Math.floor(h*.46));
...
function loadErling(){
  startButton.disabled=true;startButton.textContent='ERLING ER PÅ VEJ…';
  new THREE.TextureLoader().load('assets/figurer/erling-aergerlig.webp',tex=>{
    erlingTexture=makeTransparentErlingTexture(tex.image);tex.dispose();
    startButton.disabled=false;startButton.textContent='IND PÅ SKOLEN';
  },undefined,()=>{startButton.disabled=false;startButton.textContent='HENT ERLING IGEN';});
}
loadErling();
...
function createErling(){const enemy=createErlingRig(erlingTexture);enemy.group.position.copy(pickSpawnPosition());scene.add(enemy.group);enemies.push(enemy);return enemy;}
function removeEnemy(enemy){const i=enemies.indexOf(enemy);if(i>=0)enemies.splice(i,1);disposeErlingRig(enemy);scene.remove(enemy.group);}
...
function maybeSpeakWhileMoving(now){if(!enemies.length||now-lastMoveVoiceAt<6500||!('speechSynthesis' in window)||speechSynthesis.speaking)return;lastMoveVoiceAt=now;const lines=['Nu har jeg dig!','Jeg slapper din matematik-eksamen!','Du skal være et nul ligesom mig!'];speakLine(lines[moveVoiceIndex++%lines.length],{rate:.88,pitch:.67,volume:.9});}
...
startButton.addEventListener('click',()=>{if(!erlingTexture){loadErling();return;}document.getElementById('start-overlay').classList.remove('open');gameActive=true;ensureMusic();audioCtx?.resume();clearEnemies();newProblem();spawnWave(1,true);controls.lock();});
