...
export class Match {
  ...
  input(id, data) {
    ...
    const v = data.pose;
    if (p.hp > 0 && v && finite(v.x,v.y,v.z,v.yaw) && Math.abs(v.x)<26.5 && Math.abs(v.z)<26.5 && v.y>=1.7 && v.y<3.5) {
      const length = distance(p,v);
      const allowance = 5.4*Math.min(.8,Math.max(0,this.clock-p.poseAt))+.15;
      ...
      for (let i=1;clear && i<=steps;i++) clear = !this.blocked(p.x+(v.x-p.x)*i/steps,p.z+(v.z-p.z)*i/steps,.48);
      if (clear) { p.x=v.x; p.y=v.y; p.z=v.z; p.yaw=v.yaw; p.poseAt=this.clock; }
    }
  ...
  tick(dt) {
    ...
    for (const s of this.shots) {
      ...
      for (let i=0;i<steps && s.life>0 && this.phase==='playing';i++) {
        ...
        if (s.y<0 || s.y>3.7 || this.blocked(s.x,s.z,.1)) { s.life=0; break; }
        if (this.mode==='deathmatch') {
          for (const p of this.players.values()) if (p.id!==s.owner && p.hp>0 && distance(s,p)<.55 && s.y>p.y-1.7 && s.y<p.y+.25) {
