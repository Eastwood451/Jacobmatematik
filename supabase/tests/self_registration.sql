...
update public.registration_settings set enabled = true, teacher_id = 'e9892ea2-6818-46d0-8720-568a9f91c101' where id;
...
select pg_temp.assert_true((select role = 'student' and teacher_id = 'e9892ea2-6818-46d0-8720-568a9f91c101' and self_registered and username = 'test-søren-9'
...
select pg_temp.assert_true((select count(*) = 1 from public.results), 'Administrator sees own unclassified student results');
select pg_temp.assert_true((select jsonb_array_length(data->'users') = 1 from public.school_state), 'New signup joins the normal roster');
select pg_temp.assert_true(public.list_self_registered('test-søren-9')->0->>'assigned' = 'false', 'Teacher ownership does not imply class placement');
-- The same exercise settings are saved in the same roster and sent to the student.
update public.school_state set data = jsonb_set(data, '{users,0,assignedTables}', '[2,3]');
select set_config('request.jwt.claim.sub', 'e9892ea2-6818-46d0-8720-568a9f91c103', true);
select pg_temp.assert_true(public.get_my_student_state()->'users'->0->'assignedTables' = '[2,3]'::jsonb, 'Unclassified student receives exercise settings');
select pg_temp.assert_true(public.get_my_student_state()->'users'->0->>'classId' is null, 'Student is not automatically placed in a class');
select set_config('request.jwt.claim.sub', 'e9892ea2-6818-46d0-8720-568a9f91c101', true);
...
select pg_temp.assert_true(public.list_self_registered('test-søren-9')->0->>'assigned' = 'true', 'Registry reflects class placement');
select pg_temp.assert_true((select data->'users'->0->'assignedTables' = '[2,3]'::jsonb from public.school_state), 'Class placement preserves exercise settings');
select public.assign_self_registered('e9892ea2-6818-46d0-8720-568a9f91c103', 'test-7');
